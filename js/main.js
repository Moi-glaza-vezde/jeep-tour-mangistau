...
const infoMore = document.querySelectorAll('.tours__info-more');
console.log(infoMore);
//туры
burger.addEventListener('click', function () {
   nav.classList.add('header__nav-list--active');

   if (nav.closest('.header__nav-list--active')) {
...
infoMore.forEach(function (item) {
   item.addEventListener('click', function () {
      item.parentElement.closest('.tours__item').classList.toggle('tours__item--active');
...
const lengArr = {
   ...
   routeList4Item3: {
      ru: 'озеро Куздыкары ( озеро в виде сердца )',
      en: 'Lake Kuzdikary ( lake in the form of a heart )',
      qz: 'Құмдықары көлі (жүрек түріндегі көл)',
   },
   routeList4Item4: {
      ru: 'Протяжёность 350 км, 10 часов',
      en: 'Length 350 km, 10 hours',
   ...
   toursList3Item6: {
      ru: 'каньон Саура ( небольшое пресноводное озерцо с черепашками )',
      en: 'Saura Canyon ( a small freshwater lake with turtles )',
      qz: 'Каньон Саура (тасбақалары бар шағын Тұщы су көлі )',
   },
   toursList3Item7: {
   ...
   toursList4Item10: {
      ru: 'каньон Каратау ( чёрный взъерошенный каньён )',
      en: 'Karatau Canyon ( black ruffled canyon )',
      qz: 'Қаратау каньоні (қара бұдырлы каньен )',
   },
   toursList4Item11: {
...
function changLang() {
   ...
   select.value = hash;
   document.querySelector('title').innerHTML = lengArr['title'][hash];

   for (let key in lengArr) {
      document.querySelectorAll('.lang-' + key).forEach(function (item) {
         item.innerText = lengArr[key][hash];
      });
   }
}
changLang();
